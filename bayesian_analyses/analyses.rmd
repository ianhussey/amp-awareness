...
# # what's with the constrained ranges on the density plots?
# ggplot(combined_data, aes(influence_rate, color = experiment, fill = experiment)) + 
...
#   scale_fill_viridis_d(end = 0.8)
# 
# ggplot(combined_data, aes(absolute_int_amp_effect, color = experiment, fill = experiment)) + 
#   geom_density(alpha = 0.3) +
#   theme_classic() +
#   ylim(0, 1) +
#   scale_colour_viridis_d(end = 0.8) +
...
model_path    <- "models/fit_1_weak_prior"
...
prior         <- c(set_prior("normal(0, 1)"))  # weakly informative prior used on all parameters, see https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations
model_path    <- "models/fit_2_weak_prior"
...
# model specific setup
data          <- exp_2_ia_amp_task_level_data
prior         <- c(prior(normal(0.1653846,   # mean(fit_1_posterior$b_Intercept)
                                0.02381052), # sd(fit_1_posterior$b_Intercept)
                         class = Intercept), 
                   prior(normal(0.3680013,   # mean(fit_1_posterior$b_influence_rate)
                                0.05225875), # sd(fit_1_posterior$b_influence_rate)
                         class = b),
                   prior(normal(0.2187121,   # mean(fit_1_posterior$sigma)
                                0.01279158), # sd(fit_1_posterior$sigma)
                         class = sigma))
model_path    <- "models/fit_2_informed_prior"
...
# Savage-Dickey Bayes Factor (BF10)
H2_sav_dic         <- fit_2 %>% hypothesis(hypothesis = "influence_rate = 0", alpha = .05)  
...
# fit_2_loo <- loo(fit_2, fit_2_weak_prior)
# 
# fit_2_loo
...
prior         <- c(set_prior("normal(0, 1)"))  # weakly informative prior used on all parameters, see https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations
model_path    <- "models/fit_3_weak_prior"
...
# model specific setup
data          <- exp_3_ia_amp_task_level_data
prior         <- c(prior(normal(0.122227,     # mean(fit_2_posterior$b_Intercept)
                                0.0188327),   # sd(fit_2_posterior$b_Intercept)
                         class = Intercept), 
                   prior(normal(0.3688868,    # mean(fit_2_posterior$b_influence_rate)
                                0.0370128),   # sd(fit_2_posterior$b_influence_rate)
                         class = b),
                   prior(normal(0.2202041,    # mean(fit_2_posterior$sigma)
                                0.009032641), # sd(fit_2_posterior$sigma)
                         class = sigma))
model_path    <- "models/fit_3_informed_prior"
...
# Savage-Dickey Bayes Factor (BF10)
H3_sav_dic         <- fit_3 %>% hypothesis(hypothesis = "influence_rate = 0", alpha = .05)  
...
Savage-Dickey Bayes Factor (BF10) (with mu = 0) = `r round(1/H3_sav_dic$hypothesis$Evid.Ratio, 3)`.
...
# fit_3_loo <- loo(fit_3, fit_3_weak_prior)
# 
# fit_3_loo
...
data          <- combined_data
prior         <- c(set_prior("normal(0, 1)"))  # weakly informative prior used on all parameters, see https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations
model_path    <- "models/fit_combination_weak_prior"
...
model_path    <- "models/fit_combination_very_weak_prior"
