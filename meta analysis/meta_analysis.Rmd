...
# find differences
#setdiff(colnames(exp_2), colnames(exp_1))
...
fixed_effects_exp_1_part <- summary(model_exp_1_part)$coefficients %>%
  ...
  filter(effect == "(Intercept)") %>%     # only the intercept
  mutate(Experiment = 1)
...
Total N = `r nrow(combined_participant_level_data)` participants.
...
model_exp_1_part <- combined_participant_level_data %>%
  filter(experiment == 1) %>%
  lm(IA_AMP_effect_positive_negative ~ influence_rate, 
     data = .)
...
fixed_effects_exp_1_part <- summary(model_exp_1_part)$coefficients %>%
  as.data.frame() %>%
  rownames_to_column(var = "effect") %>%
  filter(effect == "influence_rate") %>%      # only the influence rate
  mutate(Experiment = 1)

fixed_effects_exp_2_part <- summary(model_exp_2_part)$coefficients %>%
  as.data.frame() %>%
  rownames_to_column(var = "effect") %>%
  filter(effect == "influence_rate") %>%
  mutate(Experiment = 2)
...
fixed_effects_exp_1_amp <- summary(model_exp_1_amp)$coefficients %>%
  as.data.frame() %>%
  rownames_to_column(var = "effect") %>%
  filter(effect == "prime_typePositive") %>%      # only prime_typePositive
  mutate(Experiment = 1)
...
Total N = `r nrow(combined_participant_level_data)` participants, `r nrow(combined_trial_level_data)` total evaluations on the IA-AMPs.
