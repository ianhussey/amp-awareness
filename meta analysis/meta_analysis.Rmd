...
combined_trial_level_data <- read.csv("data/combined_trial_level_data.csv") %>%
  mutate(influenced = case_when(influenced == 1 ~ 0,
                                influenced == 0 ~ 1))

long_participant_level_data <- 
  combined_participant_level_data %>%
  filter(experiment != 1 & experiment != 4) %>%
  select(unique_id, experiment, AMP_effect_positive_negative,
         AMP_effect_obama_trump, influence_rate) %>%
  gather(AMP_type, AMP_effect, 
         AMP_effect_positive_negative:AMP_effect_obama_trump) %>%
  na.omit()

different_domains_data <- combined_participant_level_data %>%
  filter(experiment == 3 | experiment == 4) %>%
  select(unique_id, experiment, IA_AMP_effect_positive_negative,
         AMP_effect_obama_trump,  IA_AMP_effect_obama_trump) %>%
  gather(amp_type, AMP_effect_politics, 
         IA_AMP_effect_obama_trump:AMP_effect_obama_trump) %>%
  na.omit() %>%
  select(-amp_type) %>%
  mutate(AMP_effect_politics = abs(AMP_effect_politics))
...
#### (rates of) rates of influence

```{r}
# mean and sd for influence rates
combined_participant_level_data %>%
  summarise(mean_influence_rate = mean(influence_rate),
            sd_influence_rate = sd(influence_rate))

# proportion of participants with influences rates...
total_participants <- nrow(combined_participant_level_data)
less_than_25 <- filter(combined_participant_level_data,
                       influence_rate < .25) %>%
  nrow()
more_than_75 <- filter(combined_participant_level_data,
                       influence_rate > .75)%>%
  nrow()

# ...less than 25%
less_than_25 / total_participants

# ...greater than 75%
more_than_75 / total_participants

```

#### Means for influenced and uninfluenced IA-AMP effects

```{r}
combined_participant_level_data %>%
  filter(!(is.na(IA_AMP_influenced_effect_positive_negative))) %>%
  dplyr::summarise(mean_influenced_effect =
                     mean(IA_AMP_influenced_effect_positive_negative),
                    sd_influenced_effect =
                     sd(IA_AMP_influenced_effect_positive_negative))

combined_participant_level_data %>%
  filter(!(is.na(IA_AMP_uninfluenced_effect_positive_negative))) %>%
  dplyr::summarise(mean_uninfluenced_effect =
                     mean(IA_AMP_uninfluenced_effect_positive_negative),
                  sd_uninfluenced_effect =
                     sd(IA_AMP_uninfluenced_effect_positive_negative))

```

### Do rates of influence correlate with any demographic variables?

```{r}
# by gender
gender_comparison <- combined_participant_level_data %>%
  filter(gender != "other")

# test
t.test(influence_rate ~ gender, data = gender_comparison,
       var.eq = TRUE)

# plot
gender_comparison %>%
  group_by(gender) %>%
  summarise(mean = mean(influence_rate),
            sd= sd(influence_rate)) %>%
  ggplot(aes(gender, mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd)) +
  labs(x = "gender", y = "mean influence rate") +
  theme_classic()


# by age
# test
cor.test(combined_participant_level_data$age,
         combined_participant_level_data$influence_rate)

# plot
ggplot(combined_participant_level_data, aes(age, influence_rate)) +
  geom_smooth(method = "lm") +
  geom_point() +
  theme_classic()
...
# by politics
party_comparison <- combined_participant_level_data %>%
  filter(!(is.na(party))) %>%
  filter(party != "neither/moderate")

# test
t.test(influence_rate ~ party, data = party_comparison)
...
party_comparison %>%
  ...
  geom_point() +
  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd)) +
  labs(x = "political party", y = "mean influence rate") +
  theme_classic()

```

### Do unrelated AMP effects correlate?

```{r}
formula <- as.formula("IA_AMP_effect_positive_negative ~ AMP_effect_politics")
effect  <- "AMP_effect_politics"

# apply models to individual experiments, extract results for key effect
results_for_meta_analysis_0 <- rbind(
  fit_lm_model_and_extract_effect(data = different_domains_data, 
                                  experiment_n = 3, formula = formula, effect = effect),
  fit_lm_model_and_extract_effect(data =  different_domains_data, 
                                  experiment_n = 4, formula = formula, effect = effect)
)

# meta analyse results
meta_results_0 <- meta_analysis_workflow(data              = results_for_meta_analysis_0, 
                                         effect_size_label = "Beta",
                                         reference_line    = 0)

```
- Meta analysis: `r meta_results_0$meta_analysis_results_text`
- Heterogeneity tests: `r meta_results_0$heterogeneity_test_results_text`
...
### Traditional AMP effect ~ influence 
...
results_for_meta_analysis_5 <- rbind(
  fit_lm_model_and_extract_effect(data = long_participant_level_data, experiment_n = 2, formula = formula, effect = effect),
  fit_lm_model_and_extract_effect(data = long_participant_level_data, experiment_n = 3, formula = formula, effect = effect),
  fit_lm_model_and_extract_effect(data = long_participant_level_data, experiment_n = 5, formula = formula, effect = effect)
)
