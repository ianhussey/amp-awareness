...
# get data
combined_participant_level_data <- read.csv("data/data_combined_participant_level.csv") 

combined_trial_level_data       <- read.csv("data/data_combined_trial_level.csv")  %>%
...
fit <- rma.mv(yi     = yi,
              ...
              data   = data_temp,
              slab   = paste("Exp", Experiment, IA_AMP_domain))
...
ggplot(temp_data, 
       aes(influence_rate, abs(IA_AMP_effect), color = IA_AMP_domain)) +
...
fit <- rma.mv(yi     = yi,
              ...
              data   = data_temp,
              slab   = paste("Exp", Experiment, domain))
...
plot_model(model_2, 
  ...
  theme_classic() +
  xlab("Influence-awareness of response") +
  ylab("Evaluation on IA-AMP") +
...
fit <- rma.mv(yi     = yi,
              ...
              data   = data_temp,
              slab   = paste("Exp", Experiment, AMP_domain))
...
temp_data <- combined_participant_level_data %>%
  filter((experiment == 2 & AMP_domain == "Generic valence") |
           ...
           (experiment == 6 & AMP_domain == "Generic valence") |
           (experiment == 7 & AMP_domain == "Generic valence")) %>%
...
p3 <- ggplot(temp_data, 
             aes(influence_rate, abs(AMP_effect), color = prediction_type)) +
  geom_smooth(method = "lm") +
  scale_color_viridis_d(end = 0.7, begin = 0.3) +
  theme_classic() +
  xlab("IA-AMP influence-awareness rate\n(Completed second)") +
