...
p <- ggplot(combined_participant_level_data, 
            aes(influence_rate, IA_AMP_effect)) +
...
# reshape data
data_temp <- results_for_meta_analysis_5 %>%
  dplyr::rename(V = vi)
...
# fit combined model
model_combined <- glmer(rating ~ prime_type * influenced + (1 | experiment/unique_id) + (1 | domain), 
                        family = binomial(link = "logit"),
...
# reshape data
data_temp <- results_for_meta_analysis_amp %>%
  dplyr::rename(V = vi)
...
combined_participant_level_data %>%
  ...
  count(influence_rate_quintiles) %>%
...
combined_participant_level_data %>%
  ...
  count(influence_rate_categories) %>%
  mutate(percent = round(n/N, 2)) %>%
  kable()
...
combined_participant_level_data %>%
  ...
  t.test(influence_rate ~ gender,
         data = .) %>%
  broom::tidy()

combined_participant_level_data %>%
  filter(gender %in% c("male", "female")) %>%
  effsize::cohen.d(influence_rate ~ gender,
                   data = .)
...
cor.test(combined_participant_level_data$influence_rate,
         combined_participant_level_data$age) %>%
  broom::tidy()

cis <- combined_participant_level_data %>%
  select(influence_rate, age) %>%
  psych::cor.ci(n.iter = 2000, plot = FALSE)
