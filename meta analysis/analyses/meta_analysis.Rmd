...
x <- full_join(
  data_participant_level_valence_ia_amp %>%
    ...
    select(unique_id, AMP_effect) %>%
    drop_na(),
  data_participant_level_valence_ia_amp %>%
    filter(IA_AMP_domain == "Generic valence") %>%
    filter(!is.na(IA_AMP_uninfluenced_effect)) %>%
    mutate(IA_AMP_uninfluenced_effect = IA_AMP_uninfluenced_effect*-1) %>%
    select(unique_id, IA_AMP_uninfluenced_effect) %>%
    drop_na(),
  by = "unique_id"
  ) 
...
ggplot(x, aes(IA_AMP_uninfluenced_effect, AMP_effect)) +
  geom_point() +
...
data_temp3 <- 
  bind_rows(
    data_participant_level_valence_ia_amp %>%
      ...
      filter(!is.na(AMP_effect)) %>%
      mutate(type = "AMP") %>%
      select(unique_id, effect = AMP_effect, type, experiment) %>%
      drop_na() %>%
      sample_n(250),
    ...
    data_participant_level_valence_ia_amp %>%
      ...
      select(unique_id, effect = IA_AMP_uninfluenced_effect, type, experiment) %>%
      drop_na() %>%
      sample_n(250)
  )
