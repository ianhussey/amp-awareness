...
h5_fit_0 <- 
  analysis_data %>% 
  select(AMP_effect_obama_trump, party) %>%
  aov(AMP_effect_obama_trump ~ party, 
      data = .)
...
h5_plot_0$data %>%
  ...
  ggplot(., aes(`Political party`, `AMP effect`)) +
...
h5_fit <- 
  analysis_data %>% 
  dplyr::select(AMP_effect_obama_trump_influenced, AMP_effect_obama_trump_not_influenced, party) %>%
  gather(influence, AMP_effect, c(AMP_effect_obama_trump_influenced, 
                                  AMP_effect_obama_trump_not_influenced)) %>%
  dplyr::mutate(influence = dplyr::recode(influence,
                                          "AMP_effect_obama_trump_not_influenced" = "Not influenced",
...
h5_plot$data.list %>%
  ...
  rename("AMP effect" = y,
         "Political party" = grp) %>%
  ggplot(., aes(Influence, `AMP effect`, color = `Political party`, shape = `Political party`)) +
...
h5_obama_trump_non_influenced <- 
  analysis_data %>% 
  select(AMP_effect_obama_trump_not_influenced, party) %>%
  na.omit() %>%
  ...
  # You bastards.
  bootES(data = .,
         data.col = "AMP_effect_obama_trump_not_influenced", 
         group.col = "party",
...
h5_obama_trump_influenced <- 
  analysis_data %>% 
  select(AMP_effect_obama_trump_influenced, party) %>%
  na.omit() %>%
  bootES(data = .,
         data.col = "AMP_effect_obama_trump_influenced", 
         group.col = "party",
         contrast = c("republican", "democrat"),  # levels must be quoted, function doesn't like numerics 
         effect.type = "cohens.d.sigma",  # cohens.d gives sample sd, adding sigma gives population sd
         ci.type = "bca",
...
# h5_fit <- 
#   analysis_data %>% 
#   select(AMP_effect_obama_trump_influenced, AMP_effect_obama_trump_not_influenced, party) %>%
#   gather(influence, AMP_effect, c(AMP_effect_obama_trump_influenced, 
#                                   AMP_effect_obama_trump_not_influenced)) %>%
#   mutate(party = recode(party,
#                         "republican" = 0,
#                         "democrat" = 1),
#          influence = recode(influence,
#                         "AMP_effect_obama_trump_not_influenced" = "not_influenced",
#                         "AMP_effect_obama_trump_influenced" = "influenced"),
#          influence = as.factor(influence)) %>%
#   glm(party ~ AMP_effect * influence, 
#       family = binomial(link = "logit"),
#       data = .)
# 
# sjt.glm(h5_fit)
