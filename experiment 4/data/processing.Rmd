...
# get data
demographics_df    <- read.csv("raw/demographics_18_10_11.csv")
AMP_df             <- read.csv("raw/modified_intentionality_amp_18_10_11.csv")
questions_df       <- read.csv("raw/post-questions_18_10_11.csv")
...
screen <- full_join(prolificid_df, self_exclusion, by= "subject") %>%
  dplyr::rename(prolificid = response.x, 
                response = response.y) %>%
  filter(response != "No, exclude my data (please specify why):") %>%
  group_by(subject) %>%
  filter(n() == 12) %>% 
  ungroup %>%
...
# Creates an amp_effect column giving the AMP effect on the politics AMP for each subject.
...
# to the post-questions, their standard AMP effect, and their politics AMP effect.
...
```{r}
# Add columns
politics_AMP_cleaner <- politics_AMP_cleaned %>%
  semi_join(full_subjects_df, by = "subject")

normed_AMP_cleaner <- normed_AMP_cleaned %>%
  semi_join(full_subjects_df, by = "subject")

```

```{r}
# Create dfs for republicans and democrats
republicans_df <- politics_AMP_cleaner %>%
  left_join(full_subjects_df, by = "subject") %>%
  filter(party == 0)

democrats_df <- politics_AMP_cleaner %>%
  left_join(full_subjects_df, by = "subject") %>%
  filter(party == 1)
...
write_csv(politics_AMP_cleaner, "processed/politics_AMP.csv")
write_csv(normed_AMP_cleaner, "processed/normed_AMP.csv")
