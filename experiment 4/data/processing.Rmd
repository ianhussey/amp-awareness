...
posneg_IA_AMP_influenced_effect <- posneg_IA_AMP_cleaned %>%
  filter(influenced == 1) %>%
  group_by(subject, prime_type) %>%
  summarize(amp_effect = mean(rating)) %>%
  ungroup() %>%
...
posneg_IA_AMP_data <- full_join(posneg_IA_AMP_effect, 
                                ...
                                by = "subject") %>%
  full_join(posneg_IA_AMP_influenced_effect, by = "subject") %>%
...
complete_data <- complete_demographics_data %>%
  ...
  semi_join(politics_IA_AMP_data, by = "subject") %>%
  semi_join(posneg_IA_AMP_data, by = "subject") %>%
