...
```{r}
iaamp_rts_df <- posneg_IA_AMP_cleaned %>%
  mutate(latency = as.numeric(as.character(latency))) %>%
  group_by(subject) %>%
...
iaamp_responses_df <- posneg_IA_AMP_cleaned %>%
  group_by(subject) %>%
  summarise(responses = sum(rating)) %>%
  filter(responses != 72,
         responses != 0)

amp_rts_df <- politics_IA_AMP_cleaned %>%
  mutate(latency = as.numeric(as.character(latency))) %>%
  group_by(subject) %>%
  summarise(mean_rt = mean(latency)) %>%
  mutate(fast = ifelse(mean_rt > 300, 0, 1)) %>%
...
amp_responses_df <- politics_IA_AMP_cleaned %>%
  group_by(subject) %>%
  summarise(responses = sum(rating)) %>%
  filter(responses != 72,
         responses != 0)
