...
# overall valence IA-AMP effect size
task_level_data_exclusions %>%
...
# specific alence IA-AMP effects
trial_level_posneg_ia_amp_data_exclusions %>%
...
# How many influence-aware trials in valence IA-AMP?
trial_level_posneg_ia_amp_data_exclusions %>%
...
# overall valence IA-AMP effect size
task_level_data_exclusions %>%
  select(subject, party, IA_AMP_effect_obama_trump) %>%
  group_by(party) %>%
  dplyr::summarise(effect = mean(IA_AMP_effect_obama_trump),
            sd = sd(IA_AMP_effect_obama_trump))

# specific alence IA-AMP effects
trial_level_politics_ia_amp_data_exclusions %>%
  group_by(subject, party, prime_type, influenced) %>%
  dplyr::summarise(mean_rating = mean(rating)) %>%
  spread(prime_type, mean_rating) %>%
  mutate(prime_type_A = ifelse(is.na(prime_type_A), .5, prime_type_A),
         prime_type_B = ifelse(is.na(prime_type_B), .5, prime_type_B)) %>%
  mutate(amp_effect = prime_type_B - prime_type_A) %>%
  select(subject, party, influenced, amp_effect) %>%
  spread(influenced, amp_effect) %>%
  mutate(`0` = ifelse(is.na(`0`), 0, `0`),
         `1` = ifelse(is.na(`1`), 0, `1`)) %>%
  group_by(party) %>%
  dplyr::summarise(mean_uninfluenced = mean(`0`),
            sd_uninfluenced = sd(`0`),
            mean_influenced = mean(`1`),
            sd_influenced = sd(`1`))
...
# How many influence-aware trials in valence IA-AMP?
trial_level_politics_ia_amp_data_exclusions %>%
  dplyr::count(subject, party, influenced) %>%
  spread(influenced, n) %>%
  mutate(`0` = ifelse(is.na(`0`), 0, `0`),
         `1` = ifelse(is.na(`1`), 0, `1`)) %>%
  group_by(party) %>%
  dplyr::summarise(mean_influenced = mean(`1`),
            sd = sd(`1`))
...
p1 <- ggplot() +
  ...
  scale_x_continuous(breaks = c(-1.0, -0.5, 0.0, 0.5, 1.0),
                     labels = c("-1.0\n(Obama-good/Trump-bad)", "-0.5", "0.0", "0.5", "1.0\n(Obama-bad/Trump-good)")) +
  ylab("Influence-awareness\n") +
  xlab("\nEvaluation on IA-AMP") +
