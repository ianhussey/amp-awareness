...
# plot
plot_model(model_h1a_1, type = "pred", terms = c("influenced", "prime_type"))
...
<<<<<<< HEAD
# extract OR
OR_h1a_1 <- exp(cbind(est=fixef(model_h1a), 
                   confint(model_h1a, parm="beta_", method="Wald")))

# convert OR to D
abs(timesavers::OR_to_d(OR_h1a_1))
=======
# extract OR and convert to d using the method proposed by Hasselblad & Hedges (1995, Equation 5, p. 170): d = log(OR)*(sqrt(3)/pi). See also Sánchez-Meca, Marín-Martínez & Chacón-Moscoso's (2003).
...
h1a_d <- c(exp(pull(coefficients_temp, Estimate)), 
  ...
  timesavers::OR_to_d(.) %>%
  round(2)
>>>>>>> c793b8738adb09a400e35d0e03c798da30db5015
...
# extract OR
OR_h1b_1 <- exp(cbind(est=fixef(model_h2b_1), 
                   confint(model_h2b_1, parm="beta_", method="Wald")))

# convert OR to D
abs(timesavers::OR_to_d(OR_h1b_1))
...
## H3: Influence rates correlate between IA-AMPs across different domains
