...
# Notes to Ian:
1. Your fourth point in the to-do list - can you double-check that you think something's off? Everything looks ok to me. Could you flag which analyses you think are off if you still find issues? 
2. Which Ruscio's A?
3. Do we have any reason for why we used 2000 comparisons in the bootstrap for H4?


# To do
4. H2, H3, H4, H6 all need scrutiny, either analyses are off or effects were not replicated/results are not as expected. In the latter case, this seems to run contrary to previous analysis implementations. - Think you might have misinterpreted this Ian, all looks good to me?
...
4. Redistribute wealth and data
...
save(model_m1_boot, file = "data/models/model_m1a_boot.RData")
load("data/models/model_m1a_boot.RData")
...
# write to disk given long runtime
save(model_m2_boot, file = "data/models/model_m1a_boot.RData")
load("data/models/model_m1a_boot.RData")
...
# add model predictions back to the original data frame
trial_level_posneg_ia_amp_data_exclusions$m1_predicted_probability <- 
  predict(model_m2, type = "response")

# predicted probability
m2_pp <- ruscios_A(data = trial_level_posneg_ia_amp_data_exclusions, 
                   variable = "m1_predicted_probability", 
                   group = "rating")
...
# apply fixef() to the output of each boot to get fixed effects, save only this to the data frame
model_h2a_1_boot <- bootMer(model_h2a_1, 
                          FUN = fixef,  
                          nsim = 1000,
                          parallel = "multicore")

# write to disk given long runtime
save(model_h2a_1_boot, file = "data/models/model_h2a_boot.RData")
load("data/models/model_h2a_1_boot.RData")
...
# add model predictions back to the original data frame
trial_level_politics_ia_amp_data_exclusions$h2a_1_predicted_probability <- 
  predict(model_h2a_1, type = "response")

# predicted probability
h2a_1_pp <- ruscios_A(data = trial_level_posneg_ia_amp_data_exclusions, 
                    variable = "h2a_1_predicted_probability", 
                    group = "rating") 

```

Model predicted probability = `r h2a_1_pp`.
...
# apply fixef() to the output of each boot to get fixed effects, save only this to the data frame
model_h2b_1_boot <- bootMer(model_h2b_1, 
                          FUN = fixef,  
                          nsim = 1000,
                          parallel = "multicore")

# write to disk given long runtime
save(model_h2b_1_boot, file = "data/models/model_h2a_boot.RData")
load("data/models/model_h2b_1_boot.RData")
...
# add model predictions back to the original data frame
trial_level_posneg_ia_amp_data_exclusions$h2b_1_predicted_probability <- 
  predict(model_h2b_1, type = "response")

# predicted probability
h2b_1_pp <- ruscios_A(data = trial_level_posneg_ia_amp_data_exclusions, 
                    variable = "h2b_1_predicted_probability", 
                    group = "rating") 

```

Model predicted probability = `r h2b_1_pp`.
...
```{r}
fit_h3a_bootstraps <- task_level_data_exclusions %>%
  ungroup() %>%
  broom::bootstrap(2000) %>%
  do(broom::tidy(lm(abs(IA_AMP_effect_positive_negative) ~ influence_rate_obama_trump, 
                    data = .)))

fit_h3a_bootstraps %>% ### Ian, I think this solution works, but I need you to check it out for me
  filter(term == "influence_rate_obama_trump") %>%
  summarize(median = quantile(as.numeric(estimate), 0.500),
            ci_lwr = quantile(as.numeric(estimate), 0.025),
            ci_upr = quantile(as.numeric(estimate), 0.975))
...
```{r}
fit_h3b_bootstraps <- task_level_data_exclusions %>%
  ungroup() %>%
  broom::bootstrap(2000) %>%
  do(broom::tidy(lm(abs(IA_AMP_effect_obama_trump) ~ influence_rate_positive_negatiev, 
                    data = .)))

fit_h3b_bootstraps %>% ### Ian, I think this solution works, but I need you to check it out for me
  filter(term == "influence_rate_positive_negative") %>%
  summarize(median = quantile(as.numeric(estimate), 0.500),
            ci_lwr = quantile(as.numeric(estimate), 0.025),
            ci_upr = quantile(as.numeric(estimate), 0.975))
