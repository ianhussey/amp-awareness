...
1. full sample hasn't been recruited yet - has been now
2. check age and party exclusions in the other studies - did that too
3. cohen's ds for h6 don't have CIs, probably needs bootstrapping. - They have CIs now since Jamie fixed bug
...
5. Redistribute wealth and data
...
# fit model
model_m1a <- glmer(rating ~ prime_type + (1 | subject), 
                  family = binomial(link = "logit"),
                  data = trial_level_posneg_ia_amp_data_exclusions)

# plot
plot_model(model_m1a, type = "pred", terms = c("prime_type"))

# results table
tab_model(model_m1a, emph.p = FALSE, ci.hyphen = ", ")
...
model_m1a_boot <- bootMer(model_m1a, 
                         FUN = fixef,  
                         nsim = 1000,
                         parallel = "multicore")

# write to disk given long runtime
save(model_m1a_boot, file = "data/models/model_m1a_boot.RData")
...
trial_level_posneg_ia_amp_data_exclusions$m1_predicted_probability <- 
  predict(model_m1a, type = "response")

# predicted probability
m1a_pp <- ruscios_A(data = trial_level_posneg_ia_amp_data_exclusions, 
                   variable = "m1_predicted_probability", 
                   group = "rating")

```

Model predicted probability = `r m1a_pp`. In this case this is the probability that a randomly selected trial preceeded by a positive prime was evaluated more positively than a randomly selected negative prime.
...
# fit model
model_m1b <- glmer(rating ~ prime_type + (1 | subject), 
                  family = binomial(link = "logit"),
                  data = trial_level_politics_ia_amp_data_exclusions)

# plot
plot_model(model_m1b, type = "pred", terms = c("prime_type"))

# results table
tab_model(model_m1b, emph.p = FALSE, ci.hyphen = ", ")
