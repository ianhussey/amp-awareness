...
fit_h3a_bootstraps <- task_level_data_exclusions %>%
  ...
  do(broom::tidy(lm(IA_AMP_effect_positive_negative ~ influence_rate_obama_trump, 
                    data = .)))
...
fit_h3b_bootstraps <- task_level_data_exclusions %>%
  ungroup() %>%
  broom::bootstrap(2000) %>%
  do(broom::tidy(lm(IA_AMP_effect_obama_trump ~ influence_rate_positive_negatiev, 
                    data = .)))
