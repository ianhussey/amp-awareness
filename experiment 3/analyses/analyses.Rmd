...
task_level_data_exclusions %>%
  select(subject, IA_AMP_effect_positive_negative, AMP_effect_obama_trump) %>%
  summarise(amp_effect = mean(AMP_effect_obama_trump),
            amp_sd = sd(AMP_effect_obama_trump),
...
## H2: IA-AMP influence rate predicts the standard AMP effect
...
fit_h2 <- task_level_data_exclusions %>%
  mutate(abs_AMP_effect_obama_trump = abs(AMP_effect_obama_trump)) %>%
  lm(abs_AMP_effect_obama_trump ~ influence_rate, data = .)

# plot
ggplot(task_level_data_exclusions, aes(influence_rate, abs(AMP_effect_obama_trump))) + 
  geom_jitter(alpha = 0.3) +
  geom_rug(position = "jitter") +
  geom_smooth(method = lm) +
