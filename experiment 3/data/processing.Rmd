...
AMP_data <- AMP_cleaned %>%
  ...
  mutate(AMP_effect_obama_trump = round(prime_type_A - prime_type_B, 2)) %>%
  select(subject, AMP_effect_obama_trump) 
...
# score influenced amp effect
IA_AMP_influenced_effect <- IA_AMP_cleaned %>%
  filter(influenced == 1) %>%
  group_by(subject, prime_type) %>%
  summarize(amp_effect = mean(rating)) %>%
  ungroup() %>%
  ...
  select(subject, IA_AMP_influenced_effect_positive_negative) 
...
IA_AMP_data <- full_join(IA_AMP_effect, 
                         IA_AMP_influence_rate, 
                         by = "subject") %>%
  full_join(IA_AMP_influenced_effect, by = "subject") %>%
...
write_csv(trial_level_amp_data, "processed/trial_level_amp_obama_trump_data.csv")
