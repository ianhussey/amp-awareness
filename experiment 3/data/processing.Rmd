...
AMP_data <- AMP_cleaned %>%
  ...
  mutate(AMP_effect_obama_trump = 
           abs(AMP_effect_obama_trump))
...
trial_level_amp_data <- processed_data %>%
  ...
  select(subject, party, prime_type, rating, self_exclusion_1, complete_data)

# long format IA AMP data
trial_level_ia_amp_data <- processed_data %>%
  right_join(IA_AMP_cleaned, by = "subject") %>%
  select(subject, party, prime_type, rating, influenced, self_exclusion_1, complete_data)
