...
data_amp_processed_bias <- AMP_cleaned %>%
  group_by(subject) %>%
  summarize(mean_response_amp_obamatrump = round(mean(rating), 3)) %>%
  ungroup() %>%
  select(subject, mean_response_amp_obamatrump)
...
AMP_data <- AMP_cleaned %>%
  ...
  mutate(AMP_effect_obama_trump = round(prime_type_B - prime_type_A, 2)) %>%
...
trial_level_amp_data <- processed_data %>%
  ...
  select(subject, party, prime_type, rating, self_exclusion_1, complete_data, prime_identity)

# long format IA AMP data
trial_level_ia_amp_data <- processed_data %>%
  right_join(IA_AMP_cleaned, by = "subject") %>%
  select(subject, party, prime_type, rating, influenced, self_exclusion_1, complete_data, prime_identity)
