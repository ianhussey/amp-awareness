...
AMP_cleaned <- AMP_df %>%
  ...
  filter(n() == 72) %>%
  ungroup() %>%
  select(subject, prime_type, rating, latency) %>%
...
AMP_data <- AMP_cleaned %>%
  ...
  mutate(AMP_effect_positive_negative = 
           abs(AMP_effect_positive_negative))
...
IA_AMP_cleaned <- IA_AMP_df %>%
  ...
  filter(n() == 72) %>%
  ungroup() %>%
  select(subject, prime_type, rating, influenced, latency) %>%
...
IA_AMP_effect <- IA_AMP_cleaned %>%
  ...
  mutate(IA_AMP_effect_positive_negative = 
...
IA_AMP_uninfluenced_effect <- IA_AMP_cleaned %>%
  ...
  mutate(IA_AMP_uninfluenced_effect_positive_negative = 
           abs(IA_AMP_uninfluenced_effect_positive_negative))
...
```{r}
iaamp_rts_df <- IA_AMP_cleaned %>%
  group_by(subject) %>%
  summarise(mean_rt = mean(latency)) %>%
  mutate(fast = ifelse(mean_rt > 300, 0, 1)) %>%
  select(-mean_rt) %>%
  filter(fast == 0)

iaamp_responses_df <- IA_AMP_cleaned %>%
  group_by(subject) %>%
  summarise(responses = sum(rating)) %>%
  filter(responses != 72,
         responses != 0)

amp_rts_df <- AMP_cleaned %>%
  group_by(subject) %>%
  summarise(mean_rt = mean(latency)) %>%
  mutate(fast = ifelse(mean_rt > 300, 0, 1)) %>%
  select(-mean_rt) %>%
...
processed_data <- demographics_data %>%
  ...
  semi_join(iaamp_responses_df) %>%
  semi_join(iaamp_rts_df) %>%
  semi_join(amp_responses_df) %>%
  semi_join(amp_rts_df)

# long format AMP data
trial_level_amp_data <- processed_data %>%
  right_join(AMP_cleaned, by = "subject") %>%
  select(subject, prime_type, rating, self_exclusion_1, complete_data)%>%
  semi_join(iaamp_responses_df) %>%
  semi_join(iaamp_rts_df) %>%
  semi_join(amp_responses_df) %>%
  semi_join(amp_rts_df)
