...
---
# Notes to Ian:
1. Can you add the bootstrapping code to H2 and H3?
2. Which Ruscio's A do you want to keep in the first chunk?



# To do

1. add boostrapping for H2 & H3
...
4. H2 and H3 from experiment 1 were discarded as they were not relevant to our questions in this experiment, but could notionally be assessed in retrospect (i.e., Correlation between online and offline measures of influence, 
...
```{r}
fit_h2_bootstraps <- task_level_data_exclusions %>%
  ungroup() %>%
  broom::bootstrap(2000) %>%
  do(broom::tidy(lm(abs(IA_AMP_effect_positive_negative) ~ influence_rate, 
                    data = .)))

fit_h2_bootstraps %>% ### Ian, I think this solution works, but I need you to check it out for me
  filter(term == "influence_rate") %>%
...
# t test
t.test(subset$AMP_effect_positive_negative, 
       subset$IA_AMP_uninfluenced_effect_positive_negative, 
       paired = TRUE) %>%
  tidy()

# cohen's d
effsize::cohen.d(subset$AMP_effect_positive_negative, 
       subset$IA_AMP_uninfluenced_effect_positive_negative, 
       paired = TRUE)
