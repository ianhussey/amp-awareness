...
AMP_data <- AMP_cleaned %>%
  ...
  mutate(AMP_effect_positive_negative = round(prime_type_B - prime_type_B, 2)) %>%
...
IA_AMP_effect <- IA_AMP_cleaned %>%
  ...
  ungroup() %>%
  spread(prime_type, amp_effect) %>%
  mutate(IA_AMP_effect_positive_negative = round(prime_type_B - prime_type_B, 2)) %>%
...
IA_AMP_uninfluenced_effect <- IA_AMP_cleaned %>%
  ...
  ungroup() %>%
  spread(prime_type, amp_effect) %>%
  mutate(IA_AMP_uninfluenced_effect_positive_negative = round(prime_type_B - prime_type_B, 2)) %>%
