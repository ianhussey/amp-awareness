...
# get data
input_data <-
  read.csv("../processed/subject_level.csv") %>%
  mutate(party = ifelse(party == -1, 0, party)) %>%
  rename(group = party) %>%
  na.omit %>%
  filter(self_exclusion_1 == "Yes, use my data")

input_data %>%
  mutate(politics = fct_relevel(politics,
                                "very democratic",
                                "somewhat democratic",
                                "a little democratic",
                                "a little republican",
...
fit_a <- glm(group ~ std_amp_effect, 
            family = binomial(link = "logit"),
            data = input_data) 

fit_b <- glm(group ~ std_amp_effect * influence_rate, 
            family = binomial(link = "logit"),
            data = input_data) 


fit_a_tidy <- tidy(fit_a) %>% 
  mutate(model = "group ~ std_amp_effect")

fit_b_tidy <- tidy(fit_b) %>% 
  mutate(model = "group ~ std_amp_effect * influence_rate")


rbind(fit_a_tidy, fit_b_tidy) %>% 
  mutate(lwr = estimate - std.error*1.96,
         upr = estimate + std.error*1.96) %>%
  select(model, term, estimate, lwr, upr, p.value) %>%
...
# AUC
auc    <- auc(roc(group ~ prob, data = predictions))
auc_ci <- ci.auc(roc(group ~ prob, data = predictions), method = "bootstrap", progress = "none")

## Ruscio's A. proportion of correct ordinal rankings/probability of a randomly chosen member of one group having a higher predicted probability than the other group
...
#cor(temp_data$influence_rate, temp_data$abs_std_amp_effect)

# ggplot(temp_data, aes(influence_rate, abs_std_amp_effect)) +
#   geom_point() +
#   geom_smooth(method = "lm")
...
model_2b <- glm(group ~ std_amp_effect_minus_influence_rate, 
                family = binomial(link="logit"), 
...
# AUC
auc_2 <- auc(roc(group ~ prob, data = predictions2))
auc_ci_2 <- ci.auc(roc(group ~ prob, data = predictions2), method = "bootstrap")
...
es_2 <- effsize::cohen.d(std_amp_effect_minus_influence_rate ~ group, 
                         data = data_with_residuals)
